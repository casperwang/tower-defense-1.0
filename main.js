...
var sec = 0;
var DTI = -1;
var evolution = {todo: false, evn: 0, idx: DTI};
// enemies & towers & map
var enemies = [];
var towers = [];
var bullets = [];
...
var bombs = [];
var map = [];
var drc = [{x: 1, y: 0}, {x: -1, y: 0}, {x: 0, y: 1}, {x: 0, y: -1}];
var bfsPath = [];
for (var i = 0; i < 25; i++) {
	map[i] = new Array(25);
...
function Enemy(enemyType, x, y) {
	//level & type
	this.lvl = Math.floor(Math.pow(score, 0.64)) + 1;
	this.typ = enemyType;
	this.fze = 0;
	this.spd = 1;
	//position
	...
	this.move = function() {
		...
		if (this.drc.x * this.stp * this.spd == 0 && this.drc.y * this.stp * this.spd == 0 && this.typ != "soldier") {
			console.log(this);
		}
		this.spd = 1;
